...
const enhancedNodeCacheId = ({
  node,
  args
}) => node && node.internal && node.internal.contentDigest ? `${node.id}${node.internal.contentDigest}${JSON.stringify(args)}` : null;
...
module.exports = ({
  ...
  return Promise.all(nodes.map(node => {
    ...
    return resolveRecursive(node, fieldsToSift, type.getFields()).then(resolvedNode => {
