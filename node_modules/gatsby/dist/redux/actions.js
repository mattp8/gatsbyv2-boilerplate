...
${reservedFields.map(f => `  * "${f}"`).join(`\n`)}
  ...
  const result = _joi.default.validate(internalPage, joiSchemas.pageSchema);

  if (result.error) {
    console.log(_chalk.default.blue.bgYellow(`The upserted page didn't pass validation`));
  ...
  if (!internalPage.component.includes(`/.cache/`)) {
    ...
    if (!notEmpty || !includesDefaultExport) {
      const relativePath = path.relative(store.getState().program.directory, internalPage.component);
...
actions.createNode = (node, plugin, traceId) => {
  ...
  if (node.internal.owner) {
    ...
    console.log(_chalk.default.bold.red(`The node internal.owner field is set automatically by Gatsby and not by plugin`));
    process.exit(1);
