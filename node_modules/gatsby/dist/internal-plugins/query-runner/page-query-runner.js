...
(0, _asyncToGenerator2.default)(function* () {
  ...
  yield runQueriesForPathnames(dirtyIds);
  ...
  yield runQueriesForPathnames(cleanIds);
  active = true;
  return;
});
...
function () {
  var _ref2 = (0, _asyncToGenerator2.default)(function* () {
    if (active) {
      queuedDirtyActions = _.uniq(queuedDirtyActions, a => a.payload.id);
      yield runQueriesForPathnames(findDirtyIds(queuedDirtyActions));
...
const findIdsWithoutDataDependencies = () => {
  ...
  const notTrackedIds = _.difference([...state.pages.map(p => p.path), ...[...state.staticQueryComponents.values()].map(c => c.jsonName)], [...allTrackedIds, ...seenIdsWithoutDataDependencies]); // Add new IDs to our seen array so we don't keep trying to run queries for them.
