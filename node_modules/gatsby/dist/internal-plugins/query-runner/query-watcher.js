...
exports.extractQueries = () => {
  ...
  if (process.env.NODE_ENV !== `production`) {
    watch(state.program.directory); // Ensure every component is being watched.
...
const watch = rootDir => {
  ...
  const debounceCompile = _.debounce(() => {
    queryCompiler().then(queries => {
      const components = store.getState().components; // If a component previously with a query now doesn't — update the
      // store.
  ...
  watcher = chokidar.watch(path.join(rootDir, `/src/**/*.{js,jsx,ts,tsx}`)).on(`change`, path => {
    debounceCompile();
  });
};
